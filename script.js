...
function generateSuggestions(searchText) {
  ...
  // Periode
  if (lowerText.includes("feb") || lowerText.includes("februari")) {
    suggestions.push({ text: "Februari", type: "periode", icon: "📅", display: "Periode Februari" });
  }
  if (lowerText.includes("agu") || lowerText.includes("agustus")) {
    suggestions.push({ text: "Agustus", type: "periode", icon: "📅", display: "Periode Agustus" });
  }
  ...
  if (lowerText.includes("perempuan")) {
  ...
  if (lowerText.includes("sma") && !lowerText.includes("kejuruan") && !lowerText.includes("smk")) {
    suggestions.push({ text: "SMA Umum", type: "pendidikan", icon: "🎓", display: "Pendidikan SMA Umum" });
  }
  if (lowerText.includes("smk") || lowerText.includes("kejuruan")) {
    suggestions.push({ text: "SMA Kejuruan", type: "pendidikan", icon: "🎓", display: "Pendidikan SMA Kejuruan" });
  }
  if (lowerText.includes("diploma") || lowerText.includes("d3")) {
    suggestions.push({ text: "Diploma I/II/III", type: "pendidikan", icon: "🎓", display: "Pendidikan Diploma" });
  }
  if (lowerText.includes("sarjana") || lowerText.includes("s1") || lowerText.includes("div")) {
    suggestions.push({ text: "DIV/S1/S2/S3", type: "pendidikan", icon: "🎓", display: "Pendidikan Sarjana" });
  }
  ...
  // ============================================
  // 🆕 TAMBAHAN: LAPANGAN USAHA
  // ============================================
  // Ambil semua data usaha unik dari allUsaha (sudah didefinisikan di awal)
  if (typeof allUsaha !== 'undefined' && allUsaha.length > 0) {
    // Filter usaha yang mengandung kata kunci pencarian
    const filteredUsaha = allUsaha.filter(usaha => 
      usaha.toLowerCase().includes(lowerText)
    ).slice(0, 5); // Maksimal 5 saran
    
    filteredUsaha.forEach(usaha => {
      // Potong nama usaha jika terlalu panjang untuk tampilan
      const shortUsaha = usaha.length > 40 ? usaha.substring(0, 37) + '...' : usaha;
      suggestions.push({ 
        text: usaha, 
        type: "usaha", 
        icon: "🏭", 
        display: `Usaha: ${shortUsaha}` 
      });
    });
  }
...
function applyGlobalSearch(keywords) {
  if (keywords.type === "provinsi") {
    ...
    if (currentTab === "umur") updateTabUmur();
    switchTab("umur");
    } else if (keywords.type === "usaha") 
    // Untuk pencarian Lapangan Usaha
    selectedUsaha = [keywords.text];
    renderUsahaList();
    if (currentTab === "usaha") {
      updateTabUsaha();
      updateAdditionalChartsUsaha();
    }
    // Langsung pindah ke tab Usaha
    switchTab("usaha");
}
